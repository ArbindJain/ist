...
class mirroruserController extends \BaseController {

	public function getusermirror($id)
	{
		...
		foreach ($user_videos as $key => $image) {
			...
			$user_videos[$key]->liked = Like::where('user_id', '=', $id)->where('likeable_id', '=', $image->id)->where('likeable_type','=','Video')->first();
		...
		foreach ($user_audios as $key => $image) {
			...
			$user_audios[$key]->liked = Like::where('user_id', '=', $id)->where('likeable_id', '=', $image->id)->where('likeable_type','=','Audio')->first();
		...
		$followings = null;
		if(Sentry::check()){
		$followings = (Follower::where('user_id','=', $userprofile->id)->where('following_id', '=', $id)->first()); 
		...
		foreach ($reviewaudis as $key => $value) {
			$reviewaudis[$key]->username = User::where('id','=',$value->user_id)->get();
		}
		...
		foreach($news_feeds as $newsFeed)
		{
			...
			foreach ($newarray as $key => $image) {
				...
				$newarray[$key]->liked = Like::where('user_id', '=', $id)->where('likeable_id', '=', $image->postid)->where('likeable_type','=',$image->model)->first();
		...
		$connect_request = Connect::where('user_id','=',$id)
							->where('connect_id','=',$id)
							->where('status','=','1')
							->first();
							
		$connected = Connect::where('user_id','=',$id)
							->where('connect_id','=',$id)
							->where('status','=','2')
							->first();
		...
		return View::make('mirroruser.userprofile')
	...
	public function getausermirror_pictures($id)
	{
		...
		foreach ($album_images as $key => $image) {
			...
			$album_images[$key]->liked = Like::where('user_id', '=', $id)->where('likeable_id', '=', $image->id)->where('likeable_type','=','Picture')->first();
		...
		return View::make('mirroruser.userprofilepicture')
			->with('userprofile',$userprofile)
			->with('followings',$followings)
			->with('followingcount',$followingcount)
			->with('followedbycount',$followedbycount)
